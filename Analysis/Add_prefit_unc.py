...
def addPrefitUncertainties(hist_central, hists_up, hists_down):
    for bin_idx in range(hist_central.GetNbinsX()):
        ...
        bin_error_up_squared_total = 0.0
        bin_error_down_squared_total = 0.0
        ...
        bin_error_varied_tot = 0.5 * math.sqrt(
            bin_error_up_squared_total + bin_error_down_squared_total
        )
...
if __name__ == "__main__":
    ...
    for channel in channels:
        ...
        for region in regions:
            ...
            for (
                cat
            ) in keys_categories:  # This is inclusive/boosted/baseline/res1b/res2b
                dir_2 = dir_1.Get(cat)
                ...
                for process in samples_to_consider:
                    ...
                    if process not in dir_2.GetListOfKeys():
                        print(
                            f"Process {process} not found in {cat} for channel {channel}"
                        )
                        continue
                    ...
                    for unc in setup.weights_config["shape"]:
                        ...
                        if hist_name_up not in dir_2.GetListOfKeys():
                            print(
                                f"Process {hist_name_up} not found in {cat} for channel {channel}"
                            )
                            continue
                        ...
                        if hist_name_Down not in dir_2.GetListOfKeys():
                            print(
                                f"Process {hist_name_Down} not found in {cat} for channel {channel}"
                            )
                            continue
