...
def LeptonVeto(df, muon_pt_to_use="pt_nano"):
    ...
    df = df.Define(
        "Muon_B0",
        f"""
        v_ops::pt({muon_sel_p4}) > 20 && abs(v_ops::eta({muon_sel_p4})) < 2.4 && (Muon_mediumId && Muon_iso < 0.25)""",  # medium id and  loose iso # v_ops::pt({muon_sel_p4}) > 10 && abs(v_ops::eta({muon_sel_p4})) < 2.4 && (Muon_looseId && Muon_iso < 0.4)""",   loose id and very loose iso
