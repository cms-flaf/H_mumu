...
bkg = load(f"/afs/cern.ch/work/v/vdamante/H_mumu/Studies/validation/stuff/mini_bkgTTbar_{args.year}.root")
...
def compute_eff(data, wp1, wp2, idWP_1, idWP_2, region):
    ...
    eff_err = np.sqrt(np.sum(w_sel**2)) / max(w_total.sum(), 1)  # errore statistico pesato

    return {"count": int(w_sel.sum()), "total": int(w_total.sum()), "eff": float(eff), "err": float(eff_err)}

def compute_ssqrtb(data_sig, data_bckg, wp1, wp2, idWP_1, idWP_2, region):
    mask_sig = (
        data_sig["Signal_Fit"] & data_sig[region] &
        data_sig[f"mu1_{idWP_1}Id"] & data_sig[f"mu2_{idWP_2}Id"] &
        (data_sig["mu1_pfRelIso04_all"] < wp1) & (data_sig["mu2_pfRelIso04_all"] < wp2)
    )
    mask_bkg = (
        data_bckg["Signal_Fit"] & data_bckg[region] &
        data_bckg[f"mu1_{idWP_1}Id"] & data_bckg[f"mu2_{idWP_2}Id"] &
        (data_bckg["mu1_pfRelIso04_all"] < wp1) & (data_bckg["mu2_pfRelIso04_all"] < wp2)
    )
    ...
    s_sqrtB_err = np.sqrt((sigma_S / np.sqrt(max(B, 1)))**2 + (S * sigma_B / (2 * B**(3/2)))**2)
...
for wp1, wp2, idWP_1, idWP_2, region in product(isoWPs, isoWPs, idWPs, idWPs, regions):
    results.append({
        ...
        "sqrtDS_over_S": compute_sqrtDS_over_S(sig, bkg, wp1, wp2, idWP_1, idWP_2, region)
    })
...
json.dump(
    results,
    open(f"/afs/cern.ch/work/v/vdamante/H_mumu/Studies/validation/stuff/json/efficienciesTTbar_{args.year}.json","w"),
    indent=2
)
print(f"Efficienze salvate in /afs/cern.ch/work/v/vdamante/H_mumu/Studies/validation/stuff/json/efficienciesTTbar_{args.year}.json")
...
# json.dump(
#     results,
#     open(f"/afs/cern.ch/work/v/vdamante/H_mumu/Studies/validation/stuff/efficiencies_{args.year}.json","w"),
#     indent=2
# )
